...
func LoadOBJFile(filename string, mat st.Material) []st.Hittable {
	...
	for _, obj := range d.Objects {
		for _, face := range obj.Faces {
			...
			if numVertices == 4 {
				...
				tri1 := st.NewTriangle(st.Vec3{X: float64(v1[0]), Y: float64(v1[1]), Z: float64(v1[2])}, st.Vec3{X: float64(v2[0]), Y: float64(v2[1]), Z: float64(v2[2])}, st.Vec3{X: float64(v3[0]), Y: float64(v3[1]), Z: float64(v3[2])}, mat)
